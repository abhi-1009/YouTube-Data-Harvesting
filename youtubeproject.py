...
if channel_id:
    if st.button("Enter"):
        if channel_exists(channel_id):
            st.warning("Data for this channel already exists.")
        else:
    ...
    st.info("Please enter a YouTube Channel ID to proceed.")
...
if st.button("Migrate to SQL"):
    if channel_id:
        ...
        st.error("Channel data already migrated")
        
channel_data= get_channel_info(youtube,["UCxTFPM1NYtPVk1jBwUMJcnw","UCEz0tA0xcXp-XRvRu-ZP0tA","UCxC5Ls6DwqV0e-CYcAKkExQ","UCQP57evqzgtzEcEq8neFOiw",
               "UCOnF_609TSdgcXurRRrJXIg","UClga3ybuyyjpvqsh-cf3DvQ","UCqaFXphJatlJEv2WfLtC-eg","UCmpQxK37UZ5TuDa61AZL8Yw",
               "UCn1XnDWhsLS5URXTi5wtFTA","UC5eAy9_AKthxQT7qafKcgCQ"])
channel_details=pd.DataFrame(channel_data)
channel_details

# Radio button for table selection
selected_table = st.radio(
    "Select the table for view",
...
if st.button("Collect & Show Data"):
    if channel_id:
        ...
        try:
            ...
            # Select the appropriate query based on the selected table
            if selected_table == "Channels":
                ...
                query = f'''
                SELECT * FROM Playlist WHERE channel_id = "{channel_id}"
                '''
